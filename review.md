...
| **gattrlm** | Efficient deep reasoning | Attractor models (DEQ) with GA layers. Constant memory, built-in equivariance. | Prototype; equivariance proven on rotor regression (4×–48× better extrapolation). 3D coordinate rotation: Clifford attention 1.8× better val MSE, 1.5× better OOD test MSE than standard attention (8 points). Code structure benchmark: at 8L/256d, Clifford achieves 98.24% val / 97.22% test accuracy on balanced bracket classification while standard attention fails (74.41% val / 75.00% test — majority class baseline). The advantage is conditional on task complexity and model capacity. At small scale, gap is negligible. Text quality neutral. |
