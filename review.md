...
| **gattrlm** | Efficient deep reasoning | Attractor models (DEQ) with GA layers. Constant memory, built-in equivariance. | Prototype; equivariance proven on rotor regression (4×–48× better extrapolation). 3D coordinate rotation benchmark: Clifford attention 1.8× better val MSE, 1.5× better OOD test MSE than standard attention on same architecture (8 points). Scaling to 16/32 points shows non-monotonic pattern: Clifford advantage is 1.4–1.5× on OOD at 8 and 32 points, but not at 16 points. 3D object rotation (actual mesh vertices): both models overfit to near-zero val MSE; Clifford marginally better on OOD (0.3406 vs 0.3427) but gap is negligible. Text quality neutral. |
