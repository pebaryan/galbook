...
| 2 | **gaflowlm** | RHF validated; CFS + CE tested | Rotor-based flow matching. Replaces trigonometric sphere operations with a unified algebraic framework. | RHF is numerically identical to SLERP. CFS with auxiliary CE achieves logit_ppl ≈ 1723 on wikitext-2, but a standard AR baseline at the same scale reaches PPL 248 — CFS is 7× worse. The auxiliary CE loss is necessary for AR training but not sufficient for competitive quality. |
...
- **gaflowlm**: Diagnose the CFS gap. CFS + CE is 7× worse than AR baseline at the same scale. The question is whether the gap is due to the flow objective, the multivector embedding, or the Clifford attention mechanism. Test each component in isolation.
