...
class Scene1_GeometricProduct(ThreeDScene):
    ...
    def construct(self):
        ...
        a_label = Text("a", font_size=30, color=SECONDARY, font=MONO)
        b_label = Text("b", font_size=30, color=RED, font=MONO)

        a_arrow = Arrow3D(
            np.zeros(3), a_vec, color=SECONDARY, resolution=8
        )
        b_arrow = Arrow3D(
            np.zeros(3), b_vec, color=RED, resolution=8
        )
        ...
        proj_line = DashedLine(
            start=b_vec, end=b_on_a, color=ACCENT, stroke_width=2
        )
        ...
        wedge_note = Text(
            ...
            font_size=24, color=PURPLE, font=MONO, opacity=LIGHT
        )
        ...
        formula_text = Text(
            "ab = a . b + a ^ b",
            font_size=40, color=ACCENT, weight=BOLD, font=MONO
        )
...
class Scene2_BivectorOrientation(ThreeDScene):
    ...
    def construct(self):
        ...
        # ── Title ──
        title = Text(
            "Bivector = Oriented Plane",
            font_size=44, color=PRIMARY, weight=BOLD, font=MONO
        )
        title.to_edge(UP)
        self.play(Write(title), run_time=1.5)
        self.wait(1.0)
        ...
        label_a = Text("a", font_size=28, color=SECONDARY, font=MONO)
        label_b = Text("b", font_size=28, color=RED, font=MONO)
        ...
        # Label
        ab_label = Text(
            "a ^ b", font_size=28, color=PURPLE, weight=BOLD, font=MONO
        )
        ...
        orient_label = Text(
            "orientation", font_size=22, color=ACCENT, font=MONO, opacity=LIGHT
        )
        orient_label.next_to(orient_arc, RIGHT, buff=0.3)

        self.add_subcaption("The bivector has a direction - orientation matters", duration=2)
        ...
        ba_label = Text(
            "b ^ a = -(a ^ b)",
            font_size=28, color=RED, weight=BOLD, font=MONO
        )
...
class Scene3_TrivectorVolume(ThreeDScene):
    ...
    def construct(self):
        ...
        label_a = Text("a", font_size=26, color=SECONDARY, font=MONO)
        label_b = Text("b", font_size=26, color=RED, font=MONO)
        label_c = Text("c", font_size=26, color=ACCENT, font=MONO)
        label_a.next_to(a, RIGHT, buff=0.2)
        label_b.next_to(b, UP, buff=0.2)
        ...
        vol_label = Text(
            "a ^ b ^ c  (trivector)",
            font_size=28, color=PURPLE, weight=BOLD, font=MONO
        )
        ...
        self.add_subcaption("A trivector has magnitude (volume) and orientation (handedness)", duration=2)
        self.move_camera(
            phi=55 * DEGREES, theta=-55 * DEGREES,
            run_time=3.0
        )
        self.wait(2.0)
        ...
        hierarchy = Text(
            ...
            font_size=22, color=WHITE, font=MONO, opacity=DARK_LABEL
        )
        hierarchy.to_edge(DOWN, buff=0.5)

        self.add_subcaption("Each grade encodes a different geometric object", duration=1.5)
